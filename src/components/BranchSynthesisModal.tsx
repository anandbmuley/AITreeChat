  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in select-none">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] transition-colors duration-200">
        ...
        {/* Header */}
        <div className="px-5 py-4 bg-slate-100 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Synthesize 2 independent conversation paths into a unified overview</p>
            </div>
          </div>
        ...
        <div className="p-5 overflow-y-auto space-y-5 flex-1 bg-slate-50/50 dark:bg-transparent">
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            
            {/* Branch A Selector */}
            <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3 shadow-inner">
              ...
              <select
                ...
                className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl p-2.5 text-xs text-slate-800 dark:text-slate-200 outline-none focus:border-indigo-500 transition"
              >
                {allNodes.map((node) => (
              ...
              <div className="bg-white/80 dark:bg-slate-900/60 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 space-y-1">
                <span className="font-semibold text-slate-700 dark:text-slate-300">Path Ancestry:</span>
          ...
          {synthesisResult && (
            <div className="bg-slate-50 dark:bg-slate-950 border border-indigo-200 dark:border-indigo-900/60 rounded-2xl p-5 space-y-3 shadow-inner">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
                <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                  <Sparkles className="w-4 h-4 text-amber-500" /> AI Synthesis Summary
                </span>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">Merged Context Payload</span>
              </div>
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 flex justify-end">
          <button
            onClick={onClose}
