      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="px-5 py-3.5 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs rounded-lg transition"
        ...
        <div className="p-4 bg-slate-950/60 border-b border-slate-800/80 text-xs space-y-2">
          <div className="flex items-center justify-between text-slate-300 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-4 text-[11px] font-mono text-slate-400">
              <span>Path Nodes: <strong className="text-indigo-400">{path.length}</strong></span>
              <span>Total Chars: <strong className="text-indigo-400">{totalChars}</strong></span>
              <span>Est. Tokens: <strong className="text-emerald-400">~{estimatedTokens}</strong></span>
            </div>
          </div>
          <p className="text-slate-400 text-[11px] leading-relaxed">
            This window shows the isolated chain of parent nodes collected by recursively walking <code className="bg-slate-800 text-indigo-300 px-1 py-0.5 rounded">node.parentId</code>. Unrelated branches are completely omitted to prevent token bloat and context poisoning.
          </p>
        </div>
        ...
        <div className="p-5 overflow-y-auto space-y-3 flex-1">
          {path.map((node, index) => (
            <div key={node.id} className="bg-slate-950 border border-slate-800/90 rounded-xl p-3.5 space-y-2 font-mono text-xs shadow-sm">
              ...
              <div className="flex items-center justify-between border-b border-slate-800/60 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-semibold ${node.role === 'user' ? 'text-indigo-400' : 'text-emerald-400'}`}>
                ...
                <div className="flex items-center gap-2 text-[10px] text-slate-400">
                  <span>Parent: <strong className="text-slate-300">{node.parentId || 'NULL (ROOT)'}</strong></span>
                  {index < path.length - 1 && <ArrowRight className="w-3 h-3 text-slate-400" />}
                </div>
              </div>

              <div className="bg-slate-900/90 p-3 rounded-lg border border-slate-800 text-[11px] text-slate-300 whitespace-pre-wrap leading-relaxed">
                {node.content}
              </div>
